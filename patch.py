...
content = content.replace("card.isSpecial = true;\n                    card.bText = '⭐ MYSTIC SKILL ⭐';", "card.isSpecial = true; card.isPureBonus = true;\n                    card.bText = '⭐ MYSTIC SKILL ⭐';")

content = content.replace("if (!bImgId && !bText) {", "if (cardObj.isSpecial || (!bImgId && !bText)) {")

origRenderSingleCard = """            var extraClass = cardObj.isSpecial ? 'special-aura' : '';
            var content = '';
...
newRenderSingleCard = """            var extraClass = cardObj.isSpecial ? 'special-aura' : '';
            var content = '';
            if (cardObj.isPureBonus) {
                ...
                var pokeUrl = https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/other/official-artwork/9492.png;
                content = 
...
with open('js/logic.js', 'w', encoding='utf-8') as f:
    f.write(content)
